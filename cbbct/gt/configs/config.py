# config.py - CBBCT GT-mask MIL (ablation: ground-truth localization)
import os
import sys

_GT_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
...
from paths import REPO_ROOT, THIRD_PARTY_DIR
...
PROJECT_ROOT = os.environ.get(
    "CBBCT_PROJECT_ROOT",
    "/home/huyiding/pengdie/tumor_classification",
)
NDATA_ROOT = DATA_ROOT
IMAGE_DIRS = [f"{DATA_ROOT}/train2017", f"{DATA_ROOT}/val2017"]
MASK_DIRS = [
    f"{DATA_ROOT}/panoptic_train2017",
    f"{DATA_ROOT}/panoptic_val2017",
]
WORKING_DIR = os.environ.get("CBBCT_GT_WORKING_DIR", os.path.join(_GT_DIR, "outputs"))
...
# 输出目录，所有结果都将保存在这里
OUTPUT_DIR = f"{WORKING_DIR}/kfold_results_9.19_withoutposembed_aug_globalpos"
