# config.py - CBBCT 5-fold LA-MIL experiment configuration
import os
import sys

_CBBCT_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
...
import bootstrap  # noqa: F401  # ensures shared/ is importable
from paths import DINOV2_PRETRAINED_WEIGHTS, DINOV2_REPO_PATH, REPO_ROOT, THIRD_PARTY_DIR

import torch

# --- 1. 核心路径配置 ---
# Override via environment: CBBCT_DATA_ROOT, CBBCT_WORKING_DIR
DATA_ROOT = os.environ.get(
    "CBBCT_DATA_ROOT",
    "/home/huyiding/pengdie/dino/det_datasets/4v1withouts1s2",
)
IMAGE_DIRS = [f"{DATA_ROOT}/train2017", f"{DATA_ROOT}/val2017"]
WORKING_DIR = os.environ.get(
    "CBBCT_WORKING_DIR",
    "/home/huyiding/pengdie/dino/dinov2-finetune-main/finalproject",
)

# DINOv2: set DINOV2_REPO / DINOV2_WEIGHTS env vars, or clone to third_party/dinov2
if not os.environ.get("DINOV2_WEIGHTS"):
    _default_weights = os.path.join(WORKING_DIR, "dinov2_vitl14_reg4_pretrain.pth")
    if os.path.isfile(_default_weights):
...
# 输出目录，所有结果都将保存在这里
OUTPUT_DIR = f"{WORKING_DIR}/kfold_results_withoutposembed_aug_globalpos"
#OUTPUT_DIR = '/home/huyiding/pengdie/dino/dinov2-finetune-main/finalproject/kfold_results_withoutposembed_aug_globalpos_6com_mil2'
# --- 2. 交叉验证配置 ---
K_FOLDS = 5
RANDOM_SEED = 42
...
CLS_MODEL_TYPE = "mil_asym"  # options: "mil_asym", "abmil", "dsmil", "transmil", "dtfd", "clam", "camil", "rrtmil"
...
RRT_FFN = False
RRT_MLP_RATIO = 4.0
SEG_MODEL_PATH_TEMPLATE = f"{WORKING_DIR}/kfold_results_withoutposembed_aug_globalpos/fold_{{fold_num}}/best_segmentation_model.pt"
# --- 4. 全量病人标签数据 ---
